...
void ArvBB::busca(int v){
	NoArv* a = busca_valor(this->root, v);
	if( a != NULL){
		cout <<"Valor obtido :)";
	}
	else{
...
void ArvBB::abb_altura(NoArv* a, int level) {
    if (a != NULL) {
        if (level == 1) {
            cout << "  " << a->info;
        }
        else if (level > 1) {
            abb_altura(a->esq, level-1);
            abb_altura(a->dir, level-1);
        }
    }
}
